...
(`java-platform`) pins all of them to `0.4.5` as version *constraints*, so `kandra-ktor`,
`kandra-koin`, and `kandra-codegen` are guaranteed to be from the same release without you repeating
`0.4.5` four times and risking one of them drifting out of sync on the next upgrade. This is the same
