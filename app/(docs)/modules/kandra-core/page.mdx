...
import { DOKKA_PUBLISHED_VERSION } from "@/lib/site-config";
...
<Callout kind="note">
`GeneratedUuid` and `KandraUuid` shipped in v0.4.6, after the Dokka snapshot currently published
(v{DOKKA_PUBLISHED_VERSION}) — the two links above will 404 until Dokka republishes. Same version
gap explained on [`/modules`](/modules) and [`/reference`](/reference), just visible here as a
missing page instead of a stale one.
</Callout>
